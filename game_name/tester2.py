                ...
                if effect[3] == 0:
                    if effect[0] is "Finisher":
                        if player.get_class() == "Warrior":
                            ...
                            player_attack = int(effect[1] * player_attack) + (
                            multiplier * int(effect[1] * player_attack))
...
def get_enemy():  # maybe a difficulty too?
    ...
    if not choice.isdigit():
        ...
        choice = int(choice)
    rand_hp = random.randint(6, 15) * choice
    rand_attack = random.randint(10, 20) * choice
    rand_armor = random.randint(5, 10) * choice
    if choice == 1:
        return monster.Enemy("Wolf", 2, 5, 10, 0, [15, 5, 3])
    elif choice == 2:
        return monster.Enemy("Tiger", 3, rand_hp, rand_attack, rand_armor, [20, 8, 0])
    elif choice == 3:
        return monster.Enemy("Imperial Footsoldier", 4, rand_hp + 8, rand_attack + 10, rand_armor + 10, [25, 8, 1])
    elif choice == 4:
        return monster.Enemy("Minotaur", 5, rand_hp + 15, rand_attack + 30, rand_armor + 15, [50, 12, 2])
    elif choice == 5:
        return monster.Enemy("Ez", 1, rand_hp - 29, rand_attack - 49, rand_armor, [50, 12, 2])
    else:
        return get_enemy()
...
def main():
    # create a player & an enemy, call combat
